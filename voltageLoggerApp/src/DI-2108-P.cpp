...
void DI_2108_P::softReset(){
  //if(!isInit) return;  //don't try to reset if the device isn't initialized
  cout<<this->sendMessage("reset 0")<<endl;
...
vector<double> DI_2108_P::getReadings(){
  ...
  unsigned char recieved[10000];
    
  int actual; //used to find out how many bytes were written
  
  //get data from the device
  r = libusb_bulk_transfer(dev_handle, (1 | LIBUSB_ENDPOINT_IN), recieved, 10000, &actual, 0);
